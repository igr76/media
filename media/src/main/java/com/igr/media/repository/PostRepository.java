...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(nativeQuery = true, value = "CREATE PROCEDURE  @data TIMESTAMP" +
            "SELECT * FROM (SELECT * FROM posts WHERE data > '%@data%')" +
            " LEFT JOIN user_subscriptions ON posts_authorId = user_subscriptions_index ORDER BY data")
    Collection<Post> getAllPostsNewSubscriptions(LocalDateTime data);
